...
# setup i et ord. Code-chunk med navnet setup 
# blir kjørt først hver gang en kjører en code-chunck
library(modelr)
library(knitr)
library(ggplot2)
library(ggpubr)
...
heightsPosInc$heightInt <-cut(heightsPosInc$height,
                              breaks = 8)
```
...
```{r echo=FALSE}
# forenkler litt
ggplot(heightsPosInc, mapping = aes(x = height)) +
  geom_histogram(
    stat = "bin",
    position = "stack",
    binwidth = 0.5,
...
```{r echo=FALSE}
# skrevet om til pipes og tidyverse. Bruk default clour så
# lenge fargen ikke brukes til å illustrere noe. Size 
# og alpha  kan settes for å vise hvor en har flest
# observasjoner. Jitter er også en mulighet, men den gir
# en illusjon om at vi har mer variasjon i x-variabelen
# enn vi egentlig har
heights %>% 
  filter(income > 0) %>% 
...
```{r echo=FALSE}
# la colour beskrive kjønn, så har fargen en
# funksjon. Jeg liker size og alpha slik at vi
# ser hvor observasjonene er. Får kanskje et klarere bilde hvis vi tar bort de 2% høyeste
heights %>% 
  filter(income > 0) %>% 
...
```{r echo=FALSE}
# The glory of pipes and tidyverse ;-)
heights %>% 
  filter(income > 0) %>% 
  filter(income < 300000) %>%
  filter(!is.na(education)) %>% 
  mutate(
    eduBin = cut(education, breaks = c(0,12,14,16,22), labels = c("hs", "lc", "cu", "cg"), right = TRUE)
  ) %>%
  ggplot(mapping = aes(x = height, y = income, colour = eduBin)) +
...
Det ble regnet et gjennomsnitt av de 2% høyeste inntekten (av personvernshensyn) og gjennomsnittet ble brukt til å erstatte alle verdiene i dette øverste siktet. Dette er sikkert personen som tjener langt over hva som er normalt å tjene. 
...
```{r echo=FALSE}
# Dere bør opplyse om at det har blitt gjort en log-trans
# på y aksen (inntekt)
heightsPosInc <-subset(heights, income>0)
ggplot(heightsPosInc, mapping =aes(x = height, y = income))+
    scale_y_continuous(trans = scales::log_trans())+
